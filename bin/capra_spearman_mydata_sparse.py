...
def compute_spearman(matrix1, matrix2, threshold=3):
    ...
    # If there is no variability (all values are the same), return NaN
    if np.all(matrix1_filtered == matrix1_filtered[0]) or np.all(matrix2_filtered == matrix2_filtered[0]):
        return np.nan
